...
RSpec.describe Fx::Generators::MigrationHelper do
  describe "#should_create_migration?" do
    it "returns true by default" do
      helper = described_class.new({})

      expect(helper.should_create_migration?).to be(true)
    end

    it "returns true when migration option is true" do
      helper = described_class.new(migration: true)

      expect(helper.should_create_migration?).to be(true)
    end

    it "returns false when migration option is false" do
      helper = described_class.new(migration: false)

      expect(helper.should_create_migration?).to be(false)
    end
  end
  ...
  describe "#activerecord_migration_class" do
    it "returns versioned migration class when current_version is available" do
      ...
      expect(helper.activerecord_migration_class).to eq("ActiveRecord::Migration[7.0]")
    end
